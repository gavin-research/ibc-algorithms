...
if (contrato == "alg1"){
  contractName = "Alg1B"; // Deploy Alg1
}else if (contrato === "alg2") {
  contractName = "Alg2B"; // Deploy Alg2
} else if (contrato === "alg3") {
  contractName = "Alg3B"; // Deploy Alg3
}

const config = `

const contractName = "${contractName}"; // Can be SCData (in alg3) or SCStorage (in alg2)
const SCContrato = artifacts.require(contractName);
...
const deployApp = async (deployer) => {
  ...
  await deployer.deploy(SCContrato, IBCHandler.address);
};
...
const init = async (deployer) => {
  ...
  for(const promise of [
    ...
    () => ibcHandler.bindPort(PortTransfer, SCContrato.address),
...
fs.writeFileSync("../contracts/data/migrations/2_token_migration.js", config);
